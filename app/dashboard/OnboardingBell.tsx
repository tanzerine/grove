  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" aria-haspopup="menu" aria-expanded={open} title="Notifications"
        onClick={() => setOpen((v) => !v)}
        style={{ position: 'relative', width: 38, height: 38, borderRadius: 10, border: '1px solid rgba(15,23,18,0.08)', background: 'rgba(15,23,18,0.02)', color: 'var(--gv-dim)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Icon name="bell" />
        {dotOn && (
      ...
      {open && (
        <div role="menu" style={{
          ...
          background: 'var(--gv-pop)', border: '1px solid rgba(15,23,18,0.1)', borderRadius: 14,
          ...
          {showSetup && (
            <div style={{ background: 'rgba(162,255,1,0.05)', border: '1px solid rgba(162,255,1,0.2)', borderRadius: 12, padding: 12, marginBottom: 12 }}>
              ...
              <div style={{ marginTop: 9, height: 4, borderRadius: 999, background: 'rgba(15,23,18,0.08)', overflow: 'hidden' }}>
                  ...
                  return (
                    <Link key={s.id} href={s.href} onClick={() => setOpen(false)} className="gv-nav"
                      ...
                      <span style={{ flexShrink: 0, width: 20, height: 20, borderRadius: '50%', marginTop: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10.5, fontWeight: 700,
                        background: isNext ? 'rgba(162,255,1,0.15)' : 'rgba(15,23,18,0.05)', color: isNext ? ACCENT_INK : 'var(--gv-dim)',
                        border: `1px solid ${isNext ? 'rgba(162,255,1,0.4)' : 'rgba(15,23,18,0.1)'}` }}>{s.n}</span>
                      <span style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          ...
                          {s.optional && <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--gv-dim)', background: 'rgba(15,23,18,0.05)', border: '1px solid rgba(15,23,18,0.12)', borderRadius: 5, padding: '1px 5px' }}>Optional</span>}
                        </span>
...
const KIND_META: Record<ActivityKind, { color: string; bg: string; border: string; icon: string }> = {
  writing:   { color: 'var(--gv-sky)', bg: 'rgba(15,23,18,0.12)', border: 'rgba(15,23,18,0.28)', icon: 'pen' },
  review:    { color: 'var(--gv-amber)', bg: 'rgba(15,23,18,0.12)', border: 'rgba(15,23,18,0.3)',  icon: 'reviews' },
  scheduled: { color: 'var(--gv-blue)', bg: 'rgba(15,23,18,0.12)', border: 'rgba(15,23,18,0.28)', icon: 'calendar' },
