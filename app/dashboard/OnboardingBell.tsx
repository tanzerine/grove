...
export default function OnboardingBell() {
  ...
  const dotColor = activity.attentionCount > 0 ? 'var(--gv-amber)' : ACCENT;
                  ...
                  return (
                    <Link key={s.id} href={s.href} onClick={() => setOpen(false)} className="gv-nav"
                      ...
                      <span style={{ flexShrink: 0, width: 20, height: 20, borderRadius: '50%', marginTop: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10.5, fontWeight: 700,
                        background: isNext ? 'rgba(99,194,129,0.15)' : 'rgba(255,255,255,0.05)', color: isNext ? ACCENT : 'var(--gv-dim)',
...
const KIND_META: Record<ActivityKind, { color: string; bg: string; border: string; icon: string }> = {
  writing:   { color: 'var(--gv-sky)', bg: 'rgba(127,182,230,0.12)', border: 'rgba(127,182,230,0.28)', icon: 'pen' },
  review:    { color: 'var(--gv-amber)', bg: 'rgba(224,200,120,0.12)', border: 'rgba(224,200,120,0.3)',  icon: 'reviews' },
  scheduled: { color: 'var(--gv-blue)', bg: 'rgba(155,176,232,0.12)', border: 'rgba(155,176,232,0.28)', icon: 'calendar' },
