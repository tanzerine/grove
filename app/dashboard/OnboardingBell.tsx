...
export default function OnboardingBell() {
  ...
  const remaining = onboarding.steps.filter((s) => !s.done);
  const showSetup = !onboarding.complete && remaining.length > 0;
  const items = activity.items;

  // Dot lights when there's something to act on: setup left, or a draft that
  // needs review / a failed run. (Informational events don't nag.)
