 ...
 *
 * Stateless server — the transcript lives in sessionStorage per domain and
 * rides along with each request, so no schema was added for this.
 */
...
export default function AssistantPanel() {
  ...
  const storeKey = `gv-assist:${activeId ?? 'none'}`;

  // Hydration-safe restore: open state + per-domain transcript.
  useEffect(() => {
    try { setOpen(localStorage.getItem('gv-assist-open') === '1'); } catch { /* ignore */ }
  }, []);
  useEffect(() => {
    try { setMessages(JSON.parse(sessionStorage.getItem(storeKey) ?? '[]')); } catch { setMessages([]); }
  }, [storeKey]);
  useEffect(() => {
    try { sessionStorage.setItem(storeKey, JSON.stringify(messages.slice(-30))); } catch { /* ignore */ }
  }, [messages, storeKey]);
  ...
  return (
    <>
      ...
      <aside className={`gv-assist gv-scroll ${open ? 'is-open' : ''}`} aria-hidden={!open}>
        {/* header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '16px 18px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
