...
import type { AnalyticsData, FirstPartyWindow } from '@/lib/analytics/dashboard';
import type { ServiceState, StateLevel } from '@/lib/analytics/service-state';
...
const LEVEL_COLOR: Record<StateLevel, string> = {
  ok: ACCENT,
  attention: 'var(--gv-amber)',
  off: 'var(--gv-fainter)',
};

/** Centered note a card shows instead of fabricated numbers. */
function EmptyNote({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: 'var(--gv-faint)', fontSize: 12.5, lineHeight: 1.65, padding: '24px 14px', minHeight: 130 }}>
      <span style={{ maxWidth: 260 }}>{children}</span>
    </div>
  );
}

export default function AnalyticsDashboard({
  hostname, configured, connected, verified, data, state, syncedAgo,
}: {
  hostname: string;
  configured: boolean;
  connected: boolean;
  ...
  // ── first-party window matching the selector (events cover 90 days max) ────
  const fpDays = Math.min(periodDays[period], 90);
  const fpw: FirstPartyWindow | null =
    data.firstParty?.find((w) => w.days === fpDays)
    ?? data.firstParty?.[data.firstParty.length - 1]
    ?? (data.traffic || data.answers || data.funnel
      ? { days: 90, traffic: data.traffic, answers: data.answers, funnel: data.funnel }
      : null);
  const fpOn = !liveOn && !!(fpw || data.readsSeries);
  // Any real signal at all (GSC totals OR first-party events) flips the page
  // out of the illustrative sample view.
  const anyLive = liveOn || fpOn || !!data.topContent;

  // ── real trend series, sliced to the selected period ────────────────────────
  // gsc_daily gives up to 90 days oldest→newest; 12mo just shows what we have.
  const edgeLabels = <T,>(a: T[], date: (p: T) => string) =>
    a.map((p, i) => (i === 0 || i === a.length - 1 || i === Math.floor(a.length / 2) ? date(p).slice(5) : ''));
  const sliced = (data.series ?? []).slice(-periodDays[period]);
  const liveSeries = sliced.length >= 2
    ? { clicks: sliced.map((p) => p.clicks), impr: sliced.map((p) => p.impressions), labels: edgeLabels(sliced, (p) => p.date) }
    : null;
  const readsSliced = (data.readsSeries ?? []).slice(-periodDays[period]);
  const readsSeries = readsSliced.length >= 2
    ? { views: readsSliced.map((p) => p.views), labels: edgeLabels(readsSliced, (p) => p.date) }
    : null;

  // ── sample shapes (only ever shown when NOTHING real exists, labeled) ──────
  ...
  // Real period-over-period deltas from a daily series: current window vs the
  // prior equal window. null when there isn't enough history yet → no chip.
  ...
  const pctOfPair = (curW: number[], prevW: number[]): { text: string; up: boolean } | null => {
  ...
  const pctDelta = (key: SeriesKey) => pctOfPair(...winPair(key));
  ...
  // first-party reads: % change of window view totals
  const readsDelta = (() => {
    const d = periodDays[period];
    const s = data.readsSeries ?? [];
    return pctOfPair(s.slice(-d).map((p) => p.views), s.slice(-2 * d, -d).map((p) => p.views));
  })();
  const clicksLiveDelta = pctDelta('clicks');

  // ── first-party window scalars ───────────────────────────────────────────
  const fpReads = fpw?.traffic?.total ?? sum(readsSliced.map((p) => p.views));
  const fpFunnel = fpw?.funnel ?? null;
  const fpAnswers = fpw?.answers ?? null;
  const readThrough = fpFunnel && fpFunnel.clicks > 0 ? Math.round((fpFunnel.read50 / fpFunnel.clicks) * 100) : 0;

  // ── KPI cards — GSC when verified, first-party reads otherwise, sample only
  // when nothing real exists (labeled as such in the header) ─────────────────
  ...
  const readsSpark = readsSliced.length >= 2 ? readsSliced.map((p) => p.views) : null;
  const kpiDef: Kpi[] = liveOn
    ? [
        { icon: 'rankings', label: 'Organic clicks', value: fmtCompact(live!.clicks), delta: clicksLiveDelta?.text ?? '', up: clicksLiveDelta?.up ?? true, spark: liveSpark('clicks'), fill: true },
        { icon: 'eye', label: 'Impressions', value: fmtCompact(live!.impressions), delta: pctDelta('impressions')?.text ?? '', up: pctDelta('impressions')?.up ?? true, spark: liveSpark('impressions'), fill: false },
        { icon: 'cursor', label: 'Avg. CTR', value: `${(live!.ctr * 100).toFixed(1)}%`, delta: ctrDelta()?.text ?? '', up: ctrDelta()?.up ?? true, spark: liveSpark('ctr'), fill: false },
        ...
        { icon: 'spark', label: 'AI referrals', value: fpAnswers ? fmtCompact(fpAnswers.total) : '—', delta: '', up: true, spark: null, fill: true },
      ]
    : fpOn
    ? [
        { icon: 'eye', label: 'Reads', value: fmtCompact(fpReads), delta: readsDelta?.text ?? '', up: readsDelta?.up ?? true, spark: readsSpark, fill: true },
        { icon: 'reviews', label: 'Unique readers', value: fmtCompact(fpFunnel?.clicks ?? 0), delta: '', up: true, spark: null, fill: false },
        { icon: 'book', label: 'Read past 50%', value: fmtCompact(fpFunnel?.read50 ?? 0), delta: fpFunnel ? `${readThrough}% of readers` : '', up: true, spark: null, fill: false },
        { icon: 'cursor', label: 'Conversions', value: fmtCompact(fpFunnel?.converted ?? 0), delta: '', up: true, spark: null, fill: false },
        { icon: 'spark', label: 'AI referrals', value: fmtCompact(fpAnswers?.total ?? 0), delta: '', up: true, spark: null, fill: true },
      ]
    : [
  ...
  // ── main chart: GSC clicks+impressions → first-party reads → sample ────────
  type ChartMode = 'gsc' | 'reads' | 'empty' | 'sample';
  const chartMode: ChartMode = liveSeries ? 'gsc' : readsSeries ? 'reads' : anyLive ? 'empty' : 'sample';
  const chartClicks = chartMode === 'gsc' ? liveSeries!.clicks : chartMode === 'reads' ? readsSeries!.views : cur.clicks;
  const chartImpr = chartMode === 'gsc' ? liveSeries!.impr : cur.impr.map((v, i) => Math.max(v, cur.clicks[i] + 6));
  const chartLabels = chartMode === 'gsc' ? liveSeries!.labels : chartMode === 'reads' ? readsSeries!.labels : cur.labels;
  ...
  const headlineClicks = liveOn ? fmtCompact(live!.clicks) : fpOn ? fmtCompact(fpReads) : cur.c;

  // ── traffic sources (conic donut) ──────────────────────────────────────────
  const SOURCE_COLORS = [ACCENT, 'var(--gv-sky)', 'var(--gv-dim)', 'var(--gv-fainter)'];
  const fpTraffic = fpw?.traffic ?? null;
  const sourceDef = fpTraffic
    ? fpTraffic.sources.map((s, i) => ({ name: s.name, pct: s.pct, color: SOURCE_COLORS[i] ?? 'var(--gv-fainter)' }))
    : [
  ...
  const answerSharePct = fpTraffic ? (fpTraffic.sources.find((s) => s.key === 'answer')?.pct ?? 0) : 24;
  let acc = 0;
  const donutGradient = 'conic-gradient(' + sourceDef.map((s) => { const start = acc; acc += s.pct; return `${s.color} ${start}% ${acc}%`; }).join(', ') + ')';

  // ── top posts: GSC pages → first-party per-post engagement → sample ────────
  ...
  type TableMode = 'gsc' | 'fp' | 'empty' | 'sample';
  const tableMode: TableMode = data.topContent ? 'gsc' : data.fpContent ? 'fp' : anyLive ? 'empty' : 'sample';
  const postDef: PostRow[] = tableMode === 'gsc'
    ? [...data.topContent!]
    ...
    : samplePosts;
  const tableCols = tableMode === 'fp'
    ? '1fr 110px 130px 120px 110px'
    : '1fr 110px 120px 80px 90px 110px';

  // ── rank distribution ───────────────────────────────────────────────────────
  const rankDef = data.ranking
    ? data.ranking.bands.map((b, i) => ({ label: b.label, count: b.count, color: [ACCENT, 'rgba(99,194,129,0.55)', '#7f8a86', '#3a4640'][i] ?? '#3a4640' }))
    : [
  ...
  const engines = fpAnswers
    ? fpAnswers.byEngine.map((e) => ({ name: e.name, count: String(e.count), pct: `${e.pct}%` }))
    : [
  ...
  // Header sync line — say what's actually feeding the page.
  const syncLine = !anyLive
    ? 'Sample view — connect Google to see your numbers'
    : liveOn && fpTraffic
      ? `Synced from Search Console & first-party events${syncedAgo ? ` · ${syncedAgo}` : ''}`
      : liveOn
        ? `Synced from Search Console${syncedAgo ? ` · ${syncedAgo}` : ''}`
        : 'Live first-party reader events';
      ...
      <div style={{ position: 'relative', padding: '28px 36px 48px', maxWidth: 1680, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 18 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: 'var(--gv-dim)', margin: '6px 0 0' }}>Over the {curP.range}, your content earned <span style={{ color: 'var(--gv-ink)', fontWeight: 600 }}>{headlineClicks} {liveOn || !anyLive ? 'clicks' : 'reads'}</span>
              {fpAnswers
                ? <> and <span style={{ color: 'var(--gv-ink)', fontWeight: 600 }}>{fpAnswers.total.toLocaleString()} answer-engine referrals</span>.</>
                : anyLive
                  ? <>.</>
        ...
        {/* SERVICE STATUS STRIP — what the agent + data feeds are doing right now */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 14 }}>
          {state.items.map((it) => (
            <div key={it.key} className="gv-card" style={{ background: 'var(--gv-card)', border: `1px solid ${it.level === 'attention' ? 'rgba(224,200,120,0.25)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 14, padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10.5, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--gv-faint)' }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', flexShrink: 0, background: LEVEL_COLOR[it.level], boxShadow: it.level === 'ok' ? '0 0 8px rgba(99,194,129,0.45)' : 'none' }} />
                {it.label}
              </div>
              <div style={{ fontSize: 15.5, fontWeight: 700, letterSpacing: '-0.01em', margin: '9px 0 4px', color: it.level === 'off' ? 'var(--gv-dim)' : 'var(--gv-ink)' }}>{it.value}</div>
              <div style={{ fontSize: 11.5, color: 'var(--gv-dim)', lineHeight: 1.5 }}>{it.detail}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, marginBottom: 14 }}>
          {kpiDef.map((k) => {
            ...
            return (
              <div key={k.label} className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '18px 18px 14px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginTop: 12 }}>
                  ...
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 11.5, fontWeight: 600, color: k.up ? ACCENT : 'var(--gv-red)', paddingBottom: 2 }}>{k.delta}</span>
                </div>
                {sp
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.85fr 1fr', gap: 14, marginBottom: 14 }}>
          ...
          <div className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '22px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em' }}>{chartMode === 'reads' ? 'Reader traffic' : 'Organic performance'}</div>
                <div style={{ fontSize: 12.5, color: 'var(--gv-faint)', marginTop: 3 }}>
                  {chartMode === 'reads' ? 'Daily reads from your blog’s first-party events' : 'Clicks & impressions from Google + answer engines'}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: 'var(--gv-dim)' }}><span style={{ width: 10, height: 10, borderRadius: 3, background: ACCENT }} />{chartMode === 'reads' ? 'Reads' : 'Clicks'}</div>
                {chartMode !== 'reads' && (
            ...
            <div style={{ display: 'flex', gap: 28, margin: '18px 0 6px' }}>
              {chartMode === 'reads' || fpOn ? (
                <>
                  <div>
                    <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em' }}>{fmtCompact(fpReads)}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: 'var(--gv-dim)', marginTop: 3 }}>Reads <span style={{ color: ACCENT, fontWeight: 600 }}>{readsDelta?.text ?? ''}</span></div>
                  </div>
                  <div>
                    <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--gv-soft)' }}>{fmtCompact(fpFunnel?.read50 ?? 0)}</div>
                    <div style={{ fontSize: 12, color: 'var(--gv-dim)', marginTop: 3 }}>Read past 50%</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--gv-soft)' }}>{fmtCompact(fpFunnel?.converted ?? 0)}</div>
                    <div style={{ fontSize: 12, color: 'var(--gv-dim)', marginTop: 3 }}>Conversions</div>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--gv-dim)', marginTop: 3 }}>Impressions</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--gv-soft)' }}>{kpiCtr}</div>
                    <div style={{ fontSize: 12, color: 'var(--gv-dim)', marginTop: 3 }}>Avg. CTR</div>
                  </div>
                </>
              )}
            </div>
            ...
            {chartMode === 'empty' ? (
              <div style={{ height: 226, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: 'var(--gv-faint)', fontSize: 12.5, lineHeight: 1.65 }}>
                <span style={{ maxWidth: 320 }}>
                  {liveOn
                    ? 'The daily trend appears after the next Search Console sync.'
                    : 'The daily trend appears once readers start arriving — usually within days of the first publish.'}
                </span>
              </div>
            ) : (
              <>
                <svg viewBox="0 0 640 220" preserveAspectRatio="none" style={{ width: '100%', height: 210, marginTop: 6, overflow: 'visible' }}>
                  ...
                  {chartMode !== 'reads' && <path d={iP.line} fill="none" stroke="rgba(255,255,255,0.22)" strokeWidth={2} strokeDasharray="4 5" strokeLinecap="round" />}
                  ...
                  <circle cx={cP.last.x.toFixed(1)} cy={cP.last.y.toFixed(1)} r={4.5} fill={ACCENT} stroke="var(--gv-bg)" strokeWidth={2.5} />
                </svg>
          ...
          <div className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '22px 24px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700 }}>Traffic sources</div>
            <div style={{ fontSize: 12, color: 'var(--gv-faint)', margin: '3px 0 16px' }}>Where the {curP.short.replace(' days', '-day').replace(' months', '-month')} clicks came from</div>
            {!fpTraffic && anyLive ? (
              <EmptyNote>No reader events in the {curP.range} — sources fill in as readers arrive from Google, answer engines and social.</EmptyNote>
            ) : (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
                  <div style={{ position: 'relative', width: 132, height: 132, flexShrink: 0, borderRadius: '50%', background: donutGradient }}>
                    <div style={{ position: 'absolute', inset: 19, borderRadius: '50%', background: 'var(--gv-card)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1 }}>{fpTraffic ? fmtCompact(fpTraffic.total) : cur.c}</span>
                      <span style={{ fontSize: 10, color: 'var(--gv-faint)', marginTop: 3 }}>{fpTraffic ? 'total visits' : 'total clicks'}</span>
                    </div>
                  </div>
                ...
                <div style={{ paddingTop: 18, fontSize: 11.5, color: 'var(--gv-faint)', lineHeight: 1.55, borderTop: '1px solid rgba(255,255,255,0.06)', marginTop: 20 }}>
                  <span style={{ color: ACCENT, fontWeight: 600 }}>Answer engines</span> {fpTraffic ? <>drive {answerSharePct}% of clicks to your blog.</> : <>now drive 24% of all clicks — up from 4% a quarter ago.</>}
                </div>
              </>
            )}
          </div>
        ...
        <div className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, overflow: 'hidden', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, padding: '18px 22px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <span style={{ fontSize: 12.5, color: 'var(--gv-dim)' }}>
              {tableMode === 'gsc' ? 'Top pages by Google clicks' : tableMode === 'fp' ? 'Top posts by reads (past 90 days)' : tableMode === 'sample' ? 'Sample data' : ''}
            </span>
          </div>
          ...
          {tableMode === 'empty' ? (
            <EmptyNote>
              {verified
                ? 'Page-level data lands with the next Search Console sync.'
                : 'Connect Google Search Console to see which posts win clicks — reader-level engagement appears here as events arrive.'}
            </EmptyNote>
          ) : tableMode === 'fp' ? (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: tableCols, padding: '11px 22px', fontSize: 10.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--gv-fainter)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <span>Post</span><span style={{ textAlign: 'right' }}>Reads</span><span style={{ textAlign: 'right' }}>Read past 50%</span><span style={{ textAlign: 'right' }}>Read-through</span><span style={{ textAlign: 'right' }}>Conversions</span>
              </div>
              {data.fpContent!.map((p) => (
                <div key={p.postId} className="gv-row" style={{ display: 'grid', gridTemplateColumns: tableCols, alignItems: 'center', padding: '14px 22px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, paddingRight: 18 }}>
                    ...
                    <span style={{ display: 'block', fontSize: 13.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{p.title}</span>
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 600, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{p.views.toLocaleString()}</span>
                  <span style={{ fontSize: 12.5, color: 'var(--gv-dim)', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{p.read50.toLocaleString()}</span>
                  <span style={{ fontSize: 12.5, color: 'var(--gv-soft)', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{p.views > 0 ? Math.round((p.read50 / p.views) * 100) : 0}%</span>
                  <span style={{ fontSize: 12.5, color: 'var(--gv-soft)', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{p.conversions.toLocaleString()}</span>
                </div>
              ))}
            </>
          ) : (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
          ...
          {/* rank distribution */}
          <div className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700 }}>Ranking distribution</div>
              {(data.ranking || !anyLive) && <span style={{ fontSize: 11.5, color: ACCENT, fontWeight: 600 }}>{rankTotal} terms</span>}
            </div>
            <div style={{ fontSize: 12, color: 'var(--gv-faint)', marginBottom: 18 }}>Keyword positions on Google</div>
            {!data.ranking && anyLive ? (
              <EmptyNote>
                {verified
                  ? 'Keyword positions land with the next Search Console sync.'
                  : 'Connect Google Search Console to see where every keyword ranks.'}
              </EmptyNote>
            ) : (
              <>
          ...
          <div className="gv-card" style={{ background: 'var(--gv-card-grad)', border: '1px solid rgba(99,194,129,0.16)', borderRadius: 18, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700 }}>{anyLive ? 'Answer-engine referrals' : 'Answer-engine citations'}</div>
              {!anyLive && <span style={{ fontSize: 11, color: ACCENT, fontWeight: 600 }}>▲ 41%</span>}
            </div>
            <div style={{ fontSize: 12, color: 'var(--gv-faint)', margin: '3px 0 18px' }}>{anyLive ? 'Visits that came from an answer engine' : 'Times grove content was quoted'}</div>
            {!fpAnswers && anyLive ? (
              <EmptyNote>No answer-engine visits in the {curP.range} yet. ChatGPT, Perplexity and friends show up here the moment they send a reader.</EmptyNote>
            ) : (
              <>
                ...
                <div style={{ fontSize: 11.5, color: 'var(--gv-faint)', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 14, marginTop: 18 }}>{fpAnswers ? 'Click-throughs only — Google AI Overviews report as regular Search.' : '75% of published posts are structured for AI Overviews.'}</div>
              </>
            )}
          </div>
          ...
          <div className="gv-card" style={{ background: 'var(--gv-card)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 18, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 700 }}>Content funnel</div>
            ...
            {!fpFunnel && anyLive ? (
              <EmptyNote>The funnel fills in from first-party reader events — clicks, deep reads and CTA conversions, deduped per session.</EmptyNote>
            ) : (
