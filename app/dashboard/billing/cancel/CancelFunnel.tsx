    ...
    return (
      <Shell>
        <div style={{ textAlign: 'center', padding: '24px 8px' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(99,194,129,0.14)', color: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 24 }}>✓</div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#eef1ea', margin: '0 0 8px' }}>Request received</h2>
          <p style={{ fontSize: 14.5, color: '#9aa096', lineHeight: 1.6, maxWidth: 420, margin: '0 auto 20px' }}>
          ...
          <Link href="/dashboard/billing" style={{ color: ACCENT, fontWeight: 600, fontSize: 14 }}>← Back to billing</Link>
        </div>
      </Shell>
    );
  }
    ...
    <Shell>
            ...
            {reasons.map((r) => (
              <button key={r.code} type="button" onClick={() => setReason(r.code)}
                style={{
                  ...
                  background: reason === r.code ? 'rgba(99,194,129,0.1)' : 'rgba(255,255,255,0.02)',
                  border: `1px solid ${reason === r.code ? 'rgba(99,194,129,0.5)' : LINE}`,
                  color: reason === r.code ? '#eef1ea' : '#cdd2c9',
                }}>
                <span style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0, border: `2px solid ${reason === r.code ? ACCENT : '#565a53'}`, background: reason === r.code ? ACCENT : 'transparent' }} />
                {r.label}
              </button>
            ))}
          </div>
      ...
      {step === 2 && (
        <>
          ...
          <label style={{ display: 'block', fontSize: 13.5, color: '#9aa096', margin: '18px 0 7px' }}>
      ...
      {step === 3 && (
        <>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${LINE}`, borderRadius: 12, padding: 16, marginTop: 16, fontSize: 13.5, color: '#cdd2c9' }}>
            <Row k="Plan" v={plan ? cap(plan) : '—'} />
            <Row k="Reason" v={reasons.find((r) => r.code === reason)?.label ?? '—'} />
          </div>
          {err && <p style={{ color: '#ff8585', fontSize: 13.5, marginTop: 14 }}>{err}</p>}
          <Nav>
...
function Row({ k, v }: { k: string; v: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}>
      <span style={{ color: '#9aa096' }}>{k}</span><span style={{ color: '#eef1ea' }}>{v}</span>
    </div>
  );
}
...
const Title: React.CSSProperties = { fontSize: 21, fontWeight: 700, color: '#eef1ea', margin: 0, letterSpacing: '-0.01em' };
const Sub: React.CSSProperties = { fontSize: 14, color: '#9aa096', margin: '8px 0 0', lineHeight: 1.55 };
const textArea: React.CSSProperties = { width: '100%', background: 'rgba(255,255,255,0.03)', border: `1px solid ${LINE}`, borderRadius: 11, color: '#eef1ea', fontSize: 14.5, fontFamily: 'inherit', padding: '12px 14px', outline: 'none', resize: 'vertical' };
const GhostLink: React.CSSProperties = { fontSize: 13.5, color: '#9aa096' };
const GhostBtn: React.CSSProperties = { background: 'transparent', border: 'none', color: '#9aa096', fontSize: 13.5, cursor: 'pointer', fontFamily: 'inherit', padding: 0 };
function primaryBtn(disabled: boolean): React.CSSProperties {
  return { background: ACCENT, color: '#06120b', border: 'none', borderRadius: 10, padding: '11px 20px', fontSize: 14, fontWeight: 600, cursor: disabled ? 'default' : 'pointer', fontFamily: 'inherit', opacity: disabled ? 0.5 : 1 };
}
